...
async function requestOpenAI(jsonData, chunkSize) {
  ...
  try {
    ...
    var analysis_prompt = `
    Nedan kommer en lista på olika företag som är kunder, listan innehåller också information om fakturor från dem.
     ...
     Ta intressanta rader från listan och använd som exempel i ditt svar. Svara inte med listan eller någon ny lista, utan bara med tips och råd.
     Använd gärna exempel direkt från listan.
     Svaret ska vara ett tydligt och konkret förslag på hur jag ska använda just den här informationen i en PowerBI-rapport.
     
     Tips på hur man skapar en bra PowerBI-rapport utifrån informationen:`;
    ...
    for (var i = 0; i < chunkCount; i++) {
      ...
      const completion = await openai.createCompletion({
        ...
        max_tokens: 1500,
        prompt: i == chunkCount - 1 ? analysis_prompt : preprocessing_prompt(count),
      });
